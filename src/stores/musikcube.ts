...
export const useMusikcubeStore = defineStore("musikcube", () => {
    ...
    const loginResponseHandler = (data: any) => {
        ...
        webSocketHandler.sendRequest(
            requestArtistsMessage,
            getArtistResponseHandler,
        );
        webSocketHandler.sendRequest(
            requestPlaylistsMessage,
            getPlaylistResponseHandler,
        );
    };

    const getTracksResponseHandler = (data: any) => {
        trackIDs.value = (data.options.data as Track[]).map(
            (track) => track.id,
        );

        webSocketHandler.sendRequest(
            requestTracksMetadata(trackIDs.value),
    ...
    const loadTracksForPlaylist = (playlistId: number) => {
        webSocketHandler.sendRequest(
            requestTracksByPlaylist(playlistId),
            getTracksResponseHandler,
        );
    };

    const loadTracksForArtist = (artistId: number) => {
        webSocketHandler.sendRequest(
            requestTracksByArtist(artistId),
            getTracksResponseHandler,
        );
    };
